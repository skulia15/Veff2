...
class CartoonNetworkSpinner extends React.Component {
    ...
    tick(){
        if(this.state.cartoons.length === this.state.currentImageIndex){
    ...
    render(){
        return(
            <div className= {`${styles.spinner}` } style={{ backgroundImage: `url(${this.state.cartoons[this.state.currentImageIndex]})` }}/>
        )
    }
};
